      ...
      <div className="container mx-auto px-4 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {professionalTools.map((tool, index) => (
            <Link
              key={index}
              to={tool.comingSoon ? "#" : "/chat"}
              className={`superai-card p-6 relative overflow-hidden ${
                tool.comingSoon ? 'opacity-80 cursor-default' : 'cursor-pointer'
              }`}
            >
              {tool.comingSoon && (
        ...
        <div className="mt-16 bg-muted rounded-xl p-8">
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="flex-1">
              ...
              <button className="bg-primary text-white px-6 py-3 rounded-lg font-medium shadow-sm hover:shadow-md transition-all button-effect">
